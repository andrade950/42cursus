...
Um exercício prático de redes que introduz os conceitos básicos de endereçamento TCP/IP, máscaras de sub-rede e encaminhamento.

<details>
  <summary> 🛈 Detalhes do Projeto NetPractice</summary>

- **Objetivo**: Configurar redes simuladas de pequena escala ao longo de 10 níveis, resolvendo problemas de rede através da atribuição correcta de endereços IP, máscaras de sub-rede e gateways predefinidos, de forma a que todos os dispositivos consigam comunicar.

- **Conceitos Abordados**:
  - Endereçamento TCP/IP
  - Máscaras de sub-rede (ex: 255.255.255.0, notação CIDR)
  - Gateways predefinidos e tabelas de encaminhamento
  - Routers e switches
  - Camadas do modelo OSI

- **Como Funciona**:
  - A interface de treino é iniciada localmente através do `run.sh`, que lança um servidor web e abre a página dos exercícios no browser.
  - Cada nível apresenta um diagrama de rede não funcional com campos editáveis (endereços IP, máscaras, rotas).
  - Ajusta os campos não sombreados até a configuração da rede estar correcta e verifica com **[Check again]**.
  - Exporta a configuração de cada nível concluído com **[Get my config]** e guarda-a no repositório Git.

- **Alternativa (caso o run.sh não funcione)**:
```bash
  python3 -m http.server 49242
  # Depois acede a http://localhost:49242
```

- **Submissão**:
  - 10 ficheiros de configuração exportados (um por nível) devem ser colocados na raiz do repositório.
  - O login da intranet deve ser introduzido na interface de treino antes de exportar.

- **Defesa**:
  - Durante a avaliação, é necessário completar com sucesso 3 níveis aleatórios dentro de um tempo limitado.
  - Não são permitidas ferramentas externas (é tolerado o uso de uma calculadora simples como o `bc`).

- **Desafios e Aprendizagens**:
  - Compreender na prática como funciona o endereçamento IP e a divisão em sub-redes.
  - Ler e configurar tabelas de encaminhamento correctamente.
  - Identificar pelos registos da rede porque é que um gateway está em falta ou um endereço IP é inválido.
  - Desenvolver raciocínio sistemático para diagnosticar problemas de conectividade em redes.
